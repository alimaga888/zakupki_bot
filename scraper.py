...
QUICK_REJECT_TITLE_KEYWORDS = [
    ...
    # Инженерные / строительные объекты
    "строительств мост",
    "реконструкц мост",
    "путепровод",
    ...
    # Неархитектурные услуги, которые уже встретились
...
def build_work_profile(
    ...
    if re.search(
        r"(?:разработ\w*|подготов\w*|выполн\w*).{0,120}"
        r"(?:графическ\w+\s+материал|чертеж\w*)",
        focus_text,
        re.IGNORECASE
    ):
...
with sync_playwright() as p:
    ...
    if missing_codes:
        ...
        # Возвращаем общий поиск семейства
        try:
